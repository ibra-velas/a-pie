...
**The per-subcategory palette is hand-picked by Ibrahim** (June 2026), like the city coordinates — don't normalize it back to group colors. `SUB_COLORS` in `Map.jsx` holds the chosen colors (cafe coffee brown, comida_rapida McDonald's red, heladeria candy pink, fresh-food shops apple green…); `SUB_MARKER_STYLES` holds custom-drawn markers (farmacia green cross on white/black ring, restaurante white utensils on orange, peluqueria white scissors on black, supermercado near-black SVG cart on apple green — the 🛒 emoji rendered low-contrast on mobile, ferreteria light face with silver ring so the gray 🔧 emoji stands out). Entries without `glyph` keep the emoji and only restyle the disc. Sidebar pills/headers reuse these colors via `textTone()` in `Sidebar.jsx`, which darkens by perceived luminance for text legibility — light tones must go through it.
